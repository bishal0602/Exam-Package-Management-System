...
const Session = () => {
  ...
  useEffect(() => {
    fetch(import.meta.env.VITE_BACKEND_URL + "API/query/getOldArchives")
      .then((res) => {
        if (res.status === 200) {
          res.json().then((json) => {
            setOldArchives(json);
  ...
  const startNewSession = () => {
    ...
    if (archiveCofirm) {
      fetch(import.meta.env.VITE_BACKEND_URL + "API/query/addNewSession", {
        method: "POST",
        headers: {
  ...
  const loadSession = () => {
    ...
    if (loadCofirm) {
      fetch(import.meta.env.VITE_BACKEND_URL + "API/query/loadSession", {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
